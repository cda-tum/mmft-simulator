...
private:
    ...
    Eigen::MatrixXd Aclean; // matrix A = [G, B; C, D]
    Eigen::VectorXd z;      // vector z = [i; e]
    Eigen::VectorXd x;      // vector x = [v; j]
    Eigen::VectorXd x_prev; // vector x^(n-1)
    Eigen::VectorXd lambda; // vector of (eigen frequency)^2
    ...
    std::vector<std::vector<T>> xn;     // x_n
    std::vector<std::vector<T>> xn_dt;  // x_dot_n (1st "time" derivative)
    std::vector<std::vector<T>> xn_dt2; // x_double dot_n (2nd "time" derivative)
    std::vector<std::vector<T>> lambdan; // x_double dot_n (2nd "time" derivative)
    std::vector<std::vector<T>> Alphan; // x_double dot_n (2nd "time" derivative)
