...
class Membrane : public Edge<T> {
  private:
    ...
    std::shared_ptr<Channel<T>> channel = nullptr;  ///< Pointer to the channel this membrane is attached to (length must be equal).
    std::shared_ptr<Tank<T>> tank = nullptr;        ///< Pointer to the tank this membrane is attached to (length must be equal).
  ...
  public:
    ...
    */
    [[nodiscard]] std::shared_ptr<Tank<T>> getTank() const;
