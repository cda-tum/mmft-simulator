...
template<typename T>
T MembraneModel0<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
...
template<typename T>
T MembraneModel0<T>::getPoreResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture) const {
...
template<typename T>
T MembraneModel1<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
...
template<typename T>
T MembraneModel1<T>::getPoreExitResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return membrane->getHeight() / (diffusionCoefficient * M_PI * pow(membrane->getPoreRadius(), 2));
}

template<typename T>
T MembraneModel1<T>::getPoreResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (4 * diffusionCoefficient * membrane->getPoreRadius());
}

template<typename T>
T MembraneModel1<T>::getPoreDensityDependentResistance(arch::Membrane<T> const* membrane, T area, T diffusionCoefficient) const {
...
template<typename T>
T MembraneModel2<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    T diffusionCoefficient = specie->getDiffusivity();
...
template<typename T>
T MembraneModel2<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T area, T freeDiffusionCoefficient) const {
    return (2 * freeDiffusionCoefficient / membrane->getArea()) * membrane->getPoreRadius() * membrane->getNumberOfPores(area);
}

template<typename T>
T MembraneModel2<T>::getTransmembraneResistance(arch::Membrane<T> const* membrane, T reducedDiffusionCoefficient, T area) const {
...
template<typename T>
T MembraneModel3<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    T diffusionCoefficient = specie->getDiffusivity();
...
template<typename T>
T MembraneModel3<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (4 * membrane->getPoreRadius() * diffusionCoefficient);
}

template<typename T>
T MembraneModel3<T>::getPorePassageResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient, T area) const {
...
template<typename T>
T MembraneModel4<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    T diffusionCoefficient = specie->getDiffusivity();
...
template<typename T>
T MembraneModel4<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (4 * membrane->getPoreRadius() * diffusionCoefficient);
}

template<typename T>
T MembraneModel4<T>::getPorePassageResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
...
template<typename T>
T MembraneModel5<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    T diffusionCoefficient = specie->getDiffusivity();
    return (getPoreDiscoveryResistance(membrane, diffusionCoefficient) + getPorePassageResistance(membrane, diffusionCoefficient)) / membrane->getNumberOfPores(area);
}

template<typename T>
T MembraneModel5<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (4 * membrane->getPoreRadius() * 2 * diffusionCoefficient);
}

template<typename T>
T MembraneModel5<T>::getPorePassageResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
...
template<typename T>
T MembraneModel6<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    T diffusionCoefficient = specie->getDiffusivity();
...
template<typename T>
T MembraneModel7<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
...
template<typename T>
T MembraneModel7<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (2 * membrane->getPoreRadius() * diffusionCoefficient);
}

template<typename T>
T MembraneModel7<T>::getPorePassageResistance(arch::Membrane<T> const* membrane, T area, T diffusionCoefficient) const {
...
template<typename T>
T MembraneModel8<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    auto diffusionCoefficient = specie->getDiffusivity();
...
template<typename T>
T MembraneModel8<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (2 * membrane->getPoreRadius() * diffusionCoefficient);
}

template<typename T>
T MembraneModel8<T>::getPorePassageResistance(arch::Membrane<T> const* membrane, T area, T diffusionCoefficient) const {
    return membrane->getHeight() / (diffusionCoefficient * area);
}
...
template<typename T>
T MembraneModel9<T>::getMembraneResistance(arch::Membrane<T> const* membrane, Mixture<T> const* mixture, Specie<T> const* specie, T area) const {
    auto diffusionCoefficient = specie->getDiffusivity();
    return (getPoreDiscoveryResistance(membrane, diffusionCoefficient) / membrane->getNumberOfPores(area) + getPorePassageResistance(membrane, area, diffusionCoefficient));
}

template<typename T>
T MembraneModel9<T>::getPoreDiscoveryResistance(arch::Membrane<T> const* membrane, T diffusionCoefficient) const {
    return 1 / (4 * membrane->getPoreRadius() * diffusionCoefficient);
}

template<typename T>
T MembraneModel9<T>::getPorePassageResistance(arch::Membrane<T> const* membrane, T area, T diffusionCoefficient) const {
    return membrane->getHeight() / (diffusionCoefficient * area);
}
}   /// namespace sim
