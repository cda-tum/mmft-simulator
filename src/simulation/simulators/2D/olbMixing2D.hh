...
template<typename T>
lbmMixingSimulator2D<T>::lbmMixingSimulator2D (
    ...
    T epsilon_, T relaxationTime_, T adRelaxationTime_) : 
        lbmSimulator2D<T>(id_, name_, stlFile_, cfdModule_, openings_, resistanceModel_, charPhysLength_, charPhysVelocity_, resolution_, epsilon_, 
...
lbmMixingSimulator2D<T>::lbmMixingSimulator2D (
    ...
    T charPhysVelocity_, T resolution_, T epsilon_, T relaxationTime_, T adRelaxationTime_) : 
        lbmMixingSimulator2D<T>(id_, name_, stlFile_, cfdModule_, openings_, updateScheme_, resistanceModel_, charPhysLength_, charPhysVelocity_, resolution_, epsilon_, 
...
template<typename T>
void lbmMixingSimulator2D<T>::lbmInit (T dynViscosity, T density) {
...
template<typename T>
void lbmMixingSimulator2D<T>::prepareGeometry () {
...
template<typename T>
void lbmMixingSimulator2D<T>::prepareLattice () {
...
template<typename T>
void lbmMixingSimulator2D<T>::setBoundaryValues (int iT) {
...
template<typename T>
void lbmMixingSimulator2D<T>::solve() {
    // theta = 10
...
template<typename T>
void lbmMixingSimulator2D<T>::nsSolve() {
    // theta = 10
    this->setBoundaryValues(this->step);
    for (int iT = 0; iT < 10; ++iT){
        this->collideAndStream();
...
template<typename T>
void lbmMixingSimulator2D<T>::adSolve() {
    // theta = 10
    this->setBoundaryValues(this->step);
...
template<typename T>
bool lbmMixingSimulator2D<T>::hasAdConverged() const {
    bool c = true;
...
template<typename T>
void lbmMixingSimulator2D<T>::writeVTK (int iT) {
...
template<typename T>
std::unordered_map<int, T> lbmMixingSimulator2D<T>::getAdOmegas() {
...
template<typename T>
void lbmMixingSimulator2D<T>::initValueContainers () {
...
template<typename T>
void lbmMixingSimulator2D<T>::initAdConvergenceTracker () {
...
template<typename T>
void lbmMixingSimulator2D<T>::initAdConverters (T density) {
...
template<typename T>
void lbmMixingSimulator2D<T>::prepareAdLattice (const T adOmega, int speciesId) {
...
template<typename T>
void lbmMixingSimulator2D<T>::initConcentrationIntegralPlane() {
...
template<typename T>
void lbmMixingSimulator2D<T>::initAdLattice(int adKey) {
...
template<typename T>
void lbmMixingSimulator2D<T>::prepareCoupling() {
...
template<typename T>
void lbmMixingSimulator2D<T>::executeCoupling() {
    this->lattice->executeCoupling();
}

template<typename T>
void lbmMixingSimulator2D<T>::collideAndStreamAD() {
    for (auto& [speciesId, adLattice] : adLattices) {
...
template<typename T>
void lbmMixingSimulator2D<T>::setConcentration2D (int key) {
    ...
    if (this->flowRates.at(key) >= 0.0) {
        for (auto& [speciesId, adLattice] : adLattices) {
            ...
            olb::AnalyticalConst2D<T,T> inConc(this->concentrations.at(key).at(speciesId));
...
template<typename T>
void lbmMixingSimulator2D<T>::storeCfdResults (int iT) {
    int input[1] = { };
    T output[10];
...
template<typename T>
void lbmMixingSimulator2D<T>::storeAdResults (int nodeId, int iT) {
    int input[1] = { };
    T output[10];
