...
void Droplet<T>::addDropletResistance(const ResistanceModel<T>& model) {
    ...
    if (isInsideSingleChannel()) {
        ...
        channel->addDropletResistance(model.getDropletResistance(channel, this->shared_from_this(), volumeInsideChannel));
    } else {
        // loop through boundaries
        for (auto& boundary : boundaries) {
            auto channel = boundary->getChannelPosition().getChannel();
            channel->addDropletResistance(model.getDropletResistance(channel, this->shared_from_this(), boundary->getVolume()));
        }

        // loop through fully occupied channels (if present)
        for (auto& channel : channels) {
            channel->addDropletResistance(model.getDropletResistance(channel, this->shared_from_this(), channel->getVolume()));
        }
    }
}
...
template<typename T>
void Droplet<T>::removeBoundary(std::shared_ptr<DropletBoundary<T>> boundaryReference) {
    // TODO: remove more than one boundary at once (remove_if)

    for (unsigned int i = 0; i < boundaries.size(); i++) {
        if (boundaries[i].get() == boundaryReference.get()) {
...
std::vector<std::shared_ptr<DropletBoundary<T>>> Droplet<T>::getConnectedBoundaries(int nodeId, std::shared_ptr<DropletBoundary<T>> doNotConsider) {
    ...
    for (auto& boundary : boundaries) {
        ...
        if (boundary.get() == doNotConsider.get()) {
            continue;
        }
