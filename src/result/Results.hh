...
SimulationResult<T>::SimulationResult(  std::shared_ptr<arch::Network<T>> network_,
                                        std::unordered_map<int, std::shared_ptr<sim::Fluid<T>>> fluids_,
                                        std::unordered_map<int, std::shared_ptr<sim::Droplet<T>>> droplets_) :
...
void SimulationResult<T>::addState(T time, std::unordered_map<int, T> pressures, std::unordered_map<int, T> flowRates) {
    int id = states.size();
    std::shared_ptr<State<T>> newState = std::make_unique<State<T>>(id, time, pressures, flowRates);
...
void SimulationResult<T>::addState(T time, std::unordered_map<int, T> pressures, std::unordered_map<int, T> flowRates, std::unordered_map<int, sim::DropletPosition<T>> dropletPositions) {
    int id = states.size();
    std::shared_ptr<State<T>> newState = std::make_unique<State<T>>(id, time, pressures, flowRates, dropletPositions);
    states.push_back(newState);
}
...
void SimulationResult<T>::addState(T time, std::unordered_map<int, T> pressures, std::unordered_map<int, T> flowRates, std::unordered_map<int, std::deque<sim::MixturePosition<T>>> mixturePositions) {
    ...
    std::shared_ptr<State<T>> newState = std::make_unique<State<T>>(id, time, pressures, flowRates, mixturePositions, filledEdges);
    states.push_back(newState);
}
