...
struct SimulationResult {
    ...
    std::shared_ptr<std::unordered_map<int, sim::Fluid<T>>> fluids;                 /// Contains all fluids which were defined (i.e., also the fluids which were created when droplets merged).
    std::shared_ptr<std::unordered_map<int, sim::Droplet<T>>> droplets;             /// Contains all droplets that occurred during the simulation not only the once that were injected (i.e., also merged and splitted droplets)
    std::unordered_map<int, std::shared_ptr<sim::Mixture<T>>> mixtures;
    std::shared_ptr<std::unordered_map<int, sim::Specie<T>>> species;
    ...
    SimulationResult(   std::shared_ptr<arch::Network<T>> network, 
                        std::shared_ptr<std::unordered_map<int, sim::Fluid<T>>> fluids, 
                        std::shared_ptr<std::unordered_map<int, sim::Droplet<T>>> droplets);
