...
TEST(Continuous, triangleNetwork) {
    // define simulation 1
    std::shared_ptr<sim::Simulation<T>> testSimulationPtr1 = std::make_shared<sim::Simulation<T>>();
    sim::Simulation<T> testSimulation1 = *testSimulationPtr1;
    testSimulation1.setType(sim::Type::Abstract);
    testSimulation1.setPlatform(sim::Platform::Continuous);

    // define simulation 2
    std::shared_ptr<sim::Simulation<T>> testSimulationPtr2 = std::make_shared<sim::Simulation<T>>();
    sim::Simulation<T> testSimulation2 = *testSimulationPtr2;
    testSimulation2.setType(sim::Type::Abstract);
    testSimulation2.setPlatform(sim::Platform::Continuous);

    // define network 1
    std::shared_ptr<arch::Network<T>> networkPtr1 = std::make_shared<arch::Network<T>>();
    arch::Network<T> network1 = *networkPtr1;
    testSimulation1.setNetwork(networkPtr1);

    // define network 2
    std::shared_ptr<arch::Network<T>> networkPtr2 = std::make_shared<arch::Network<T>>();
    arch::Network<T> network2 = *networkPtr2;
    testSimulation2.setNetwork(networkPtr2);
    ...
    // Define and set the resistance model
    std::shared_ptr<sim::ResistanceModel1D<T>> resistanceModel1 = std::make_shared<sim::ResistanceModel1D<T>>(testSimulation1.getContinuousPhase()->getViscosity());
    testSimulation1.setResistanceModel(resistanceModel1);
    std::shared_ptr<sim::ResistanceModel1D<T>> resistanceModel2 = std::make_shared<sim::ResistanceModel1D<T>>(testSimulation2.getContinuousPhase()->getViscosity());
    ...
    // results
    result::SimulationResult<T>* result1 = testSimulation1.getSimulationResults().get();
    result::SimulationResult<T>* result2 = testSimulation2.getSimulationResults().get();
